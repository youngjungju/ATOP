...
def extract_visual_observations(image_bytes: bytes, processor, model, device: str) -> list[str]:
    ...
    prompt = """
당신은 피부 관찰 보조 시스템입니다.
이미지에서 보이는 피부 특징만을 중립적이고 비의학적으로 요약하세요.

규칙:
- 의학적 진단, 질병명, 치료/약물 제안 금지
- 원인을 의학적 사실처럼 해석하지 말 것
- 시각적으로 관찰 가능한 내용만 설명
- 신중한 표현 사용 (“~처럼 보임”, “시각적으로 관찰됨”)

과업:
피부 이미지를 분석하여 아래 JSON 형식으로만 출력하세요.
피부를 감지하지 못하면 “피부를 감지하지 못했습니다”라고 출력하세요.
...
출력(JSON ONLY):
{
  ...
  "요약": "관찰된 피부 특징에 대한 짧은 요약",
  "일반적_설명": "이러한 특징이 일상적으로 의미하는 바에 대한 비의학적 설명",
  "한계": "이미지만으로 판단할 수 없는 점",
